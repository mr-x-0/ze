...
def get_client():
    _api = zedB.get_key("_TELEGRAPH_TOKEN")
    try:
        client = Telegraph(_api, domain="graph.org")
    except TypeError:
        client = Telegraph(_api)
    if not _api:
        gd_name = zeubot.full_name
        short_name = gd_name[:30]
        profile_url = (
            f"https://t.me/{zeubot.me.username}"
            if zeubot.me.username
            else "https://t.me/SOURCEZE"
        )
        try:
            ...
            if "SHORT_NAME_TOO_LONG" in str(er):
                client.create_account(
                    short_name="zethonuser",
                    author_name=gd_name,
                    author_url=profile_url,
                )
            LOGS.exception(er)
        if _token := client.get_access_token():
            zedB.set_key("_TELEGRAPH_TOKEN", _token)
    return client
