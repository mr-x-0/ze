...
from ze.core.helper import fast_download
from ze.core.logger import LOGS, TelethonLogger
from resources import blacklisted_users


class ZeClient(TelegramClient):
    me: User
    ...
    async def start_client(self, **kwargs):
        ...
        if self.uid in blacklisted_users:
            self.log.error(f"({me} - {self.uid}) ~ لا يمكنك أستخدام سورس أنت محظور بسبب مخالفتك سياسة أستخدام السورس @SOURCEZE")
            sys.exit(1)
        if self._log_at:
    ...
    async def send_file(self, *args, **kwargs):
        custom = zedB.get_key("CUSTOM_THUMBNAIL")
        if custom or custom is None:
            thumb = kwargs.get("thumb")
            if thumb is None:
                kwargs["thumb"] = await self._get_custom_thumb(
                    zedB.get_key("CUSTOM_THUMBNAIL") or "resources/Ze.jpg"
                )
