...
TAKE_EDITS = zedB.get_key("TAKE_EDITS")
black_list_chats = zedB.get_key("BLACKLIST_CHATS")
...
def ze_cmd(pattern=None, tgbot=tgbot, **kwargs):
    ...
    def decor(dec):
        ...
        async def wrapp(mirza: Message):
            if not zedB.get_key("DISABLE_STATS"):
            ...
            if only_devs and not zedB.get_key("I_DEV"):
                return await eod(
                    mirza,
            ...
            try:
                ...
                await zeubot.connect()
                await tgbot.send_message(
                    zedB.get_config("LOG_CHAT"), "**⌔∮ سورس زد إي يعمل بنجاح مرة أخرى**")
                return
                ...
                await tgbot.send_message(
                    ...
                    buttons=[
                        Button.url("الموقع", "http://ze.deno.dev"),
                        Button.url(
                            "البوت",
                            "https://t.me/SOURCEZE",
                        ),
                ...
                MakeHtml = f"""
<img src='resources/Ze.jpg'/>
<a href='https://D_S_IS.t.me'>أبلاغ عن المشكلة</a>
<br /><br />
<ul>
...
"""
                ...
                try:
                    ...
                    graphLink = await mkgraph("Ze Error", MakeHtml)
                    msg = f"<a href='tg://user?id={zeubot.me.id}'>\xad</a><b><a href={graphLink}>[لقـد حـدث خـطأ ⚠️]</a></b>"
        ...
        if TAKE_EDITS:
            def func_(x):
                return (
                    (x.out or x.sender_id == zeubot.me.id)
