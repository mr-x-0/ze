...
from ze.decorators import get_sudos, is_sudo
from .. import zedB, ze_cmd, zeubot, inline_mention


@ze_cmd(pattern="اضف متحكم( (.*)|$)", fullsudo=True)
async def add_sudo(e):
    ...
    if id_ == zeubot.uid:
        ...
        zedB.set_key("SUDO", "True")
        key = get_sudos()
        key.append(id_)
        zedB.set_key("SUDOS", key)
        mmm = f"**۞ المستخدم: {name}\n۞ تم بنجاح أضافته الى قائمة المتحكمين**"
    await e.eor(mmm, time=5)


@ze_cmd(pattern="حذف متحكم( (.*)|$)", fullsudo=True)
