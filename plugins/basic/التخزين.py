...
async def secpic(event):
    ...
    user_id = sender.id
    TAG_CHAT = zedB.get_config("TAG_CHAT")
    result = await event.download_media()
    caption = (
        f"**⌔∮ ميديا ذاتية التدمير وصلت لك !**\n: المرسل @{username}\nالايدي : {user_id}"
    )
    await zeubot.send_file(TAG_CHAT, result, caption=caption)


@ze_cmd(pattern="ايقاف الحفظ$")
async def _(e):
    if not e.is_private:
...
@zeubot.on(
    events.NewMessage(
        incoming=True,
        func=lambda e: e.is_private and zedB.get_key("TAG_CHAT")
    ),
)
async def permitpmhandler(event):
    ...
    if user.bot or user.is_self or user.verified or Logm.contains(user.id):
        return
    await event.forward_to(zedB.get_key("TAG_CHAT") or LOG_CHAT)
