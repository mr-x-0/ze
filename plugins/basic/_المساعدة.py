...
from ze.load_plug import PLUGINS
from database import CMD_HELP, LIST, InlinePlugin, InlinePaths
from .. import HNDLR, LOGS, tgbot, zedB, zeubot, ze_cmd, inline_pic
...
def get_from_funcs(mod, name):
    handlers = list(map(lambda d: d[0], zeubot.list_event_handlers()))
    funcs = list(
        filter(
            lambda d: (
    ...
    if not module:
        if event.client._bot:
            return await event.reply(
                "**⌔∮ مالك البوت {}\n\nالقائمة الرئيسية\n\nالأوامر ~ {}**".format(zeubot.full_name, len(PLUGINS)),
                file=inline_pic(),
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="**⌔∮ قائمة أوامر سورس زد إي**", text=text, buttons=get_help_buttons()
        )
...
async def help_func(moh):
    ...
    _strings = {
        "basic": "**⌔∮ مجموعة المساعدة: [أضغط هنا](t.me/ZESUPORT)**\n**۞ مالك الحساب هو: {}.\n۞ عدد القوائم الموجودة: {}**",
        "account": "**⌔∮ مجموعة المساعدة: [أضغط هنا](t.me/ZESUPORT)**\n**۞ مالك الحساب هو: {}.\n۞ عدد قوائم الموجودة: {}**",
        "fun": "**⌔∮ مجموعة المساعدة: [أضغط هنا](t.me/ZESUPORT)**\n**۞ مالك الحساب هو: {}.\n۞ عدد قوائم الموجودة: {}**",
        "group":"**⌔∮ مجموعة المساعدة: [أضغط هنا](t.me/ZESUPORT)**\n**۞ مالك الحساب هو: {}.\n۞ عدد قوائم الموجودة: {}**",
    }
    text = _strings.get(key, "").format(zeubot.full_name, len(plugs))
    await moh.edit(text, buttons=page_num(count, key), link_preview=False)

@callback(data="open", owner=True)
async def opner(event):
    await event.edit(
        "**⌔∮ القائمة الرئيسية لـ أوامر سورس زد إي**\n**۞ مالك الحساب هو: {}.\n۞ عدد القوائم الموجودة: {}**".format( 
            zeubot.full_name,
            len(PLUGINS),
        ),
        buttons=get_help_buttons(),
...
def _get_buttons(key, index):
    rows = zedB.get_key("HELP_ROWS") or 5
    cols = zedB.get_key("HELP_COLUMNS") or 2
    emoji = zedB.get_key("EMOJI_IN_HELP") or "۞"
