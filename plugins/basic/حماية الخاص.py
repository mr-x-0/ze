...
if isinstance(zedB.get_key("PMPERMIT"), (int, str)):
    value = [zedB.get_key("PMPERMIT")]
    zedB.set_key("PMPERMIT", value)

keym = KeySettings("PMPERMIT", cast=list)
PMPIC = zedB.get_key("PMPIC")
...
UNAPPROVED_MSG = "**نظام حماية زد إي الخاص بـ {ON}!**\n\n{UND}\n\nلديك {warn}/{twarn} تحذيرات"
if zedB.get_key("PM_TEXT"):
    UNAPPROVED_MSG = (
        "**نظام حماية زد إي الخاص بـ {ON}!**\n\n"
        + zedB.get_key("PM_TEXT")
        + "\n\nلديك {warn}/{twarn} تحذيرات"
    )

WARNS = zedB.get_key("PMWARNS") or 4
...
if zedB.get_key("PMSETTING"):
    if zedB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await zeubot.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if (sender.bot or sender.verified or sender.is_self):
            return
        inline_pm = zedB.get_key("INLINE_PM") or False
        user = event.sender
        if not keym.contains(user.id) and event.text != UND:
            if zedB.get_key("MOVE_ARCHIVE"):
                try:
                    await zeubot.edit_folder(user.id, folder=1)
                except BaseException as er:
                    LOGS.info(er)
            if event.media and not zedB.get_key("DISABLE_PMDEL"):
                await event.delete()
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    if "نظام حماية زد إي" in event.text or "**نظام حماية زد إي" in event.text:
                        return
                    ...
                    if inline_pm:
                        results = await zeubot.inline_query(
                        ...
                        _to_delete[user.id] = await zeubot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await zeubot.send_message(
                            user.id, message_
                        )
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await zeubot(BlockRequest(user.id))
                await zeubot(ReportSpamRequest(peer=user.id))
                await tgbot.edit_message(
                    zedB.get_key("LOG_CHAT"),
                    _not_approved[user.id],
    ...
    @ze_cmd(pattern="(تشغيل|ايقاف|مسح) الارشفة$", fullsudo=True)
    async def _(e):
        x = e.pattern_match.group(1).strip()
        if x == "تشغيل":
            zedB.set_key("MOVE_ARCHIVE", "True")
            await e.eor("**- من الأن سيتم نقل المستخدم الغير مسموح له الى الارشيف**", time=5)
        elif x == "ايقاف":
            zedB.set_key("MOVE_ARCHIVE", "False")
    ...
    @ze_cmd(pattern="(س|سماح)(?: |$)", fullsudo=True)
    async def approvepm(apprvpm):
        ...
        if user.id in DEVLIST:
            return await apprvpm.eor(
                "**⌔∮ هذا هو مطور سورس زد إي هو بالأصل مسموح له بالدردشة  !**",
            )
    ...
    @ze_cmd(pattern="(ر|رفض)(?: |$)", fullsudo=True)
...
@ze_cmd(pattern="بلوك( (.*)|$)", fullsudo=True)
...
@ze_cmd(pattern="الغاء البلوك( (.*)|$)", fullsudo=True)
...
@ze_cmd(pattern="قائمة المسموحين$", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await zeubot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not keym.contains(uid):
        keym.add(uid)
        try:
            await zeubot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await zeubot.get_entity(uid)
        except BaseException:
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await zeubot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await zeubot.get_entity(uid)
...
async def in_pm_ans(event):
    ...
    try:
        ...
        msg_ = "**⌔∮ نظام حماية زد إي الخاص بـ {OWNER_NAME}**"
    wrns = f"{warns}/{WARNS}"
    buttons = [
        [
        ...
        if res:
            res = [
                await event.builder.document(
                    res,
                    title="نظام حماية زد إي الأنلاين",
    ...
    await event.answer(res, switch_pm="• Ze •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[zeubot.uid])
