...
from ze.helper.tools import resize_photo_sticker

from .. import LOGS, tgbot, fetch, zedB, ze_cmd, inline_mention, create_quotly
...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"Ze Stickers of {_.sender_id}"
    typee = None
    if not local:
...
@ze_cmd(pattern="ستيكر", manager=True)
...
@ze_cmd(pattern="الحزم", manager=True)
async def do_magic(mohmd):
    jasm = zedB.get_key("STICKERS") or {}
