...
@ze_cmd(pattern="انتحال ?(.*)", fullsudo=True)
async def _(event):
    eve = await event.eor("**⌔∮ جار أنتحال المستخدم...**")
    reply_message = await event.get_reply_message()
    whoiam = await event.client(GetFullUserRequest(zeubot.uid))
    if whoiam.full_user.about:
        mybio = str(zeubot.me.id) + "01"
        zedB.set_key(f"{mybio}", whoiam.full_user.about)
    zedB.set_key(f"{zeubot.uid}02", whoiam.users[0].first_name)
    if whoiam.users[0].last_name:
        zedB.set_key(f"{zeubot.uid}03", whoiam.users[0].last_name)
...
@ze_cmd(pattern="اعادة$")
async def _(event):
    name = OWNER_NAME
    ok = ""
    mybio = str(zeubot.me.id) + "01"
    bio = zedB.get_key("MYBIO") or "خطا: ضع البايو الخاص بك"
    chc = zedB.get_key(mybio)
    if chc:
        bio = chc
    fname = zedB.get_key(f"{zeubot.uid}02")
    lname = zedB.get_key(f"{zeubot.uid}03")
    if fname:
        name = fname
    if lname:
        ok = lname
    ...
    zedB.del_key(f"{zeubot.uid}01")
    zedB.del_key(f"{zeubot.uid}02")
    zedB.del_key(f"{zeubot.uid}03")
