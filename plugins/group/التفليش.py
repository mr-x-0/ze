...
from .. import zeubot, ze_cmd

spam_chats = []


@ze_cmd(pattern="تفليش بالبوت$")
async def banavot(event):
    chat_id = event.chat_id
    try:
        await zeubot(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in zeubot.iter_participants(chat_id):
        ...
        await zeubot.send_message(chat_id, usrtxt)
